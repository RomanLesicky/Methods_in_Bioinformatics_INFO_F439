...
if 1:
    ...
    #x = range(120)
    x = range(len(depth_result_dict[depth]))
    ...
    probs_softmax = F.softmax(test_op, dim=1)
    probs_pos = probs_softmax[:, 1].tolist()    # positive class probability for AUROC/AUPRC
    probs = probs_softmax.max(1)[1].tolist()    # argmax labels for other metrics
